...
for episode = 1:10 % Number of episodes
    ...
    clear attempted_Actions;     %flushing the array attempted_Actions before next episode.
end
...
function action = Get_Next_Action(Q_values, state, epsilon, attempted_Actions)
    if rand() < epsilon
        ...
        % Exploitation: choose the best action from Q_values not in attempted Actions
        % Setting Q_values of actions belongs to attempted_Actions array to -Inf to ignore them
        possible_Actions = setdiff(1:10, attempted_Actions);
        Q_values(state(1), state(2), state(3), attempted_Actions) = -Inf;
        
        % Choose the action with the max Q_value not being -Inf
        [max_reward, action] = max(Q_values(state(1), state(2), state(3), :));
        disp('current max reward:...')
        disp(max_reward)
